...
src/lib/mod-index.ts ← mod index count fallback from latest-index index.db (build-time only)
...
The recognized-mod count uses `getModIndexStats()` at build time only as a fallback. `Features.astro` refreshes the rendered number in the browser from `https://github.com/modrexio/modrex-index/releases/download/latest-index/index-stats.json`, which is generated by `modrex-index`; do not make visitors download `index.db` just to render the count.
